...
class ItemInputModal_Single(ui.Modal):
    ...
    def __init__(self, itemname, currentvalue, maxvalue, save_stats, srvid):
        self.itemname = itemname
        self.cv = currentvalue
        self.mv = maxvalue
        self.sv = save_stats
		self.srvid = int(srvid)
        super().__init__(title="값 입력")

	async def on_submit(self, interaction: Interaction):
		self.sv = saveeditapi.CatFoodEdit(self.sv, self.inputvalue.value, interaction.user.id, self.srvid)
